...
export async function generateItinerary(req: TripRequest): Promise<Itinerary> {
  ...
  const days: ItineraryDay[] = clusters.map((group, idx) => {
    ...
    const stops = buildStops(ordered, food, scored, center, usedFood, req, wantsEvening);
    const estimatedCost =
...
function buildStops(
  ...
  const lunch = nearestWhere(anchor, food, (p) => p.category === "restaurant", usedFood);
  if (lunch) {
  ...
  const dinner = nearestWhere(last, food, (p) => p.category === "restaurant", usedFood);
  if (dinner) {
  ...
  if (provider.isLive) {
    try {
      ...
      const parsed = extractJson<{
        days: { day: number; title: string; summary: string; stops: { name: string; note: string }[] }[];
      }>(raw);
...
function applyNarration(
  days: ItineraryDay[],
  narrated: { day: number; title: string; summary: string; stops: { name: string; note: string }[] }[]
) {
  for (const d of days) {
...
function noteFor(stop: ItineraryStop): string {
  ...
  switch (p.category) {
    case "restaurant":
      return p.cuisine ? `Local ${p.cuisine} — book ahead at peak times.` : "A well-loved local table.";
    case "cafe":
