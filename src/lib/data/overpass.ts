...
/**
 * Query the free Overpass API (OpenStreetMap) for points of interest around
 * a center. Tries multiple mirrors. Returns [] on total failure so callers
 * can fall back to curated seed data.
 */
...
function toPlace(el: OverpassElement): Place | null {
  ...
  return {
    ...
    hiddenGem,
    score: isMajor ? 0.9 : category === "restaurant" || category === "cafe" ? 0.5 : 0.6,
    source: "overpass",
